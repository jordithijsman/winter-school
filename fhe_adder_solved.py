...
def full_adder(context, a, b, c_in):
    ...
    if isinstance(c_in, int):
        ...
        if c_in == 0:
            s = context.EvalBinGate(XOR, a, b)
            c_out = context.EvalBinGate(AND, a, b)
        else:
            a_xor_b = context.EvalBinGate(XOR, a, b)
            s = context.EvalNot(a_xor_b)

            a_and_b = context.EvalBinGate(AND, a, b)
            c_out = context.EvalBinGate(OR, a_and_b, a_xor_b)
    else:
        a_xor_b = context.EvalBinGate(XOR, a, b)
        s = context.EvalBinGate(XOR, a_xor_b, c_in)

        a_and_b = context.EvalBinGate(AND, a, b)
        c_in_and_a_xor_b = context.EvalBinGate(AND, a_xor_b, c_in)
        c_out = context.EvalBinGate(OR, c_in_and_a_xor_b, a_and_b)
